...
class GrantBuffer(implicit p: Parameters) extends L2Module {
  ...
  /* ======== HardwareAssertion ======== */
  val hwaFlags = Array.fill(5)(Wire(Bool()))
  for (i <- 0 until 5) {
    hwaFlags(i) := true.B
  }
...
//  }
  ...
  hwaFlags(0) := !(full && io.d_task.valid)
 ...
 /* val d_isKeyword = Mux(
  ...
  prefetchOpt.map { _ =>
    ...
    hwaFlags(1) := pftRespQueue.get.io.enq.ready
  }
  ...
  hwaFlags(2) := !(inflight_full & (io.d_task.fire && (dtaskOpcode === Grant || dtaskOpcode === GrantData || io.d_task.bits.task.mergeA)))
  ...
  when (io.e.fire) {
    hwaFlags(3) := io.e.bits.sink < grantBufInflightSize.U
  ...
  /* ======== HardwareAssertion ======== */
  HardwareAssertion(hwaFlags(0),cf"GrantBuf full and RECEIVE new task, back pressure failed")
  HardwareAssertion(hwaFlags(1),cf"pftRespQueue should never be full, no back pressure logic")
